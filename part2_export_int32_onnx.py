...
def export_int32_onnx(model_dir_or_name: str, output_path: str, max_len: int, opset: int, device: torch.device):
    print(f"[Load] {model_dir_or_name}")
    tokenizer = AutoTokenizer.from_pretrained(model_dir_or_name, use_fast=True)
    base_model = AutoModelForSequenceClassification.from_pretrained(model_dir_or_name)
    ...
    print(f"[Export] 导出 ONNX（输入=INT32）到 {output_path_with_time}")
    torch.onnx.export(
        wrapper,
    ...
    top1_ok = np.all(logits_torch.argmax(axis=1) == logits_onnx.argmax(axis=1))
...
def main():
    ...
    export_int32_onnx(args.model, args.output, args.max_len, args.opset, device)
